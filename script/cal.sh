...
awk -F'\t' 'BEGIN{OFS="\t"}NR==FNR{city[$1]=$2} NR!=FNR && ($1 in city) && !($2==city[$1]) {print $1,$2,city[$1]}' $1 $2 | wc -l
awk -F'\t' 'BEGIN{OFS="\t"}NR==FNR{city[$1]=$2} NR!=FNR && ($1 in city) && !($2==city[$1]) {print $1,$2,city[$1]}' $1 $2 > $1_$2_has_name_changed
...
awk -F'\t' 'BEGIN{OFS="\t"}NR==FNR{city[$1]=$2} NR!=FNR && !($1 in city) {print $1,$2,city[$1]}' $1 $2 > $2_has_$1_no
...
awk -F'\t' 'BEGIN{OFS="\t"}NR==FNR{city[$1]=$2} NR!=FNR && !($1 in city) {print $1,$2,city[$1]}' $2 $1 > $1_has_$2_no
